 ...
 * average point precision and 11-point interpolated precision for Bing
 */
public class MAPBing
{
    ...
    {
        MAPBing mapBing = new MAPBing();
        mapBing.readJsonfile();

        mapBing.relevalanceCheck(judgeJ,judgeL,judgeO,judgeF);
    }
    ...
    public void relevalanceCheck(ArrayList<Long> judgeJ,ArrayList<Long> judgeL,ArrayList<Long> judgeO,ArrayList<Long> judgeF) throws IOException {
    ...
    public void averagePrecision(int[] relevalance) throws IOException {
        ...
        for (int i=0;i<relevalance.length/2;i++)
        {
            if (relevalance[i] == 1 )
            {
                count += 1;
        ...
        for (int i=0;i<relevalance.length/2;i++)
        {
            ...
            {
                if (relevalance[i] == 1)
                    recall[i] = (float) (recall[i-1] + (1.0/count));
                else
                    recall[i] = recall[i-1];
            }
        }
        ...
        for (int i =0;i<relevalance.length/2;i++)
        {
            ...
            {
                    precision[i] = calcPrecision(i,relevalance);
            }
        }
    ...
    //To evaluate the 11-point interpolated average precision
    public void precision11Point(float[] recall, float[] precision,int[] relevalance) throws IOException {
